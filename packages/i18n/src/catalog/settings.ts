...
const en = {
  ...
  billingModelAria: 'Billing mode for {model} on {agent}',
  modeApi: 'Per token — what the tokens cost',
  modeSubscription: 'Token plan — a flat fee, not per token',
  modeLocal: 'Local — no cash, tokens still priced as API equivalent',
  notDeclared: 'Not declared (api default)',
  inheritAgent: 'Same as agent ({mode})',
  modelOverridden: 'own mode',
  modelsCount: '{n, plural, one {1 model} other {# models}}{overridden, plural, =0 {} other { · 1 overridden}}',
  planFee: 'Plan fee',
  perMonth: 'per month',
  saveFee: 'Save',
  feeUnknownHint: 'No fee declared, so actual cash shows only its marginal $0.',
  feeNotNumber: 'A plan fee has to be a number, or empty to undeclare it.',
  savedModelNote: '{agent} · {model} now bills as',
...
const zh = matches(en)({
  ...
  billingModelAria: '{agent} 的 {model} 计费方式',
  modeApi: '按 token —— 用 token 乘单价',
