...
function sourceFor(name: string): SourceSpec {
  ...
  return { id: deriveSourceId('pi', path), path, kind: 'jsonl', sessionHint: null }
}
...
async function readAll(
  ...
  const ctx: ParseCtx = { source, agentId: 'pi', hostId: 'pi', maxLineBytes }
  const records: RawRecord[] = []
  const stream = piAdapter.parse(source, { offset: from, firstSeq }, ctx)
  while (true) {
...
describe('parse', () => {
  it('one record per line, with a monotonic seq and byte offsets (§4.3)', async () => {
    const text = await textOf('session-trace.jsonl')
    const { records, tail } = await readAll('session-trace.jsonl')
    expect(records.map((r) => r.seq)).toEqual([1, 2, 3, 4, 5, 6])
    expect(records.map((r) => r.offset)).toEqual(records.map((_, i) => startOfLine(text, i)))
    for (const record of records) {
      expect((record.value as { id?: string }).id).toBeTruthy()
    }
    // Timestamps come from the record, so replaying an old trace keeps its own clock.
    expect(records[0]!.occurredAt).toBe(Date.parse('2026-09-20T09:00:00.000Z'))
  ...
  it('a resume continues the byte boundary and the ordinal, so raw_seq stays absolute (§5.1)', async () => {
    const all = await readAll('session-trace.jsonl')
    const third = all.records[2]!
    const resumed = await readAll('session-trace.jsonl', third.offset, third.seq)
  ...
  it('a trailing fragment without its newline is not consumed (§4.3)', async () => {
    ...
    expect(records.map((r) => (r.value as { id: string }).id)).toEqual([
      '9e1c0000-0000-4000-8000-0000000000c1',
    ])
  ...
  it('an oversized line is reported, never buffered (§4.3)', async () => {
    const { records } = await readAll('session-trace.jsonl', 0, 1, 64)
  ...
  it('a vanished source surfaces as a read error to the caller, not a silent empty scan', async () => {
    const source: SourceSpec = {
      id: deriveSourceId('pi', '/fixture/gone.jsonl'),
      path: join(FIXTURES_DIR, 'no-such-trace.jsonl'),
      kind: 'jsonl',
      sessionHint: null,
    }
    const stream = piAdapter.parse(source, { offset: 0 }, { source, agentId: 'pi', hostId: 'pi' })
...
describe('normalize of unreadable records', () => {
  it('a marker becomes a failure carrying reason/rawLine/offset/rawSeq (§5.2 rule 1)', async () => {
    ...
    for (const record of records) results.push(await piAdapter.normalize(record, ctx))
    ...
    expect(ok.map((r) => r.events.map((e) => e.type))).toEqual([['message.user'], ['message.user']])
  })
  ...
  it('a hostile record is reported, not thrown', async () => {
    ...
    const result = await piAdapter.normalize(boobyTrapped, ctx)
  ...
  it('normalize never rejects for any value JSONL can hold', async () => {
    ...
    for (const [seq, value] of scalars.entries()) {
      const result = await piAdapter.normalize({ seq, offset: seq, occurredAt: 1, value }, ctx)
  ...
  it('the fixture set round-trips through parse and normalize without a throw', async () => {
    for (const name of ['session-trace.jsonl', 'shared-usage.jsonl', 'parse-failure.jsonl']) {
      ...
      for (const record of records) await piAdapter.normalize(record, ctx)
    }
  })
})
