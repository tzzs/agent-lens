/**
 * §5.1 `discover` — one `kind:'sqlite'` source per table of `cli/db/db.sqlite`.
 *
 * WHY the id salts the table name: §4.1 defines `source_id = hash(agent_id + path)`, but
 * ZCode keeps five collected tables in ONE file, so the path alone would collapse them
 * onto a single `sources` row with a single rowid high-water mark. The qualified path
 * `{db}#{table}` is the smallest honest extension.
 *
 ...
 * already reports, so a source over them would be counted twice by any consumer.
 */
