/**
 * Shared test scaffolding: a fixture tree stands in for `~/.codex`, and every
...
/** `path` defaults to the fixture name so a test can simulate two files of one session. */
export function ctxFor(name: string, sessionHint: string | null = null, path = `/fixture/${name}`): NormalizeCtx {
  const source: SourceSpec = {
    id: deriveSourceId('codex', path),
    path,
    kind: 'jsonl',
    sessionHint,
  }
