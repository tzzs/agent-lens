/**
 * Shared test scaffolding: every case runs against a throwaway host database
 * built by `fixtures/build-host.ts`, with a fixed clock and a path-free source
...
import type { ParseTail } from '@agentlens/event-model'
...
export function hostCtx(dataRoot: string, env: NodeJS.ProcessEnv = {}): HostContext {
  return {
    dataRoot,
    homedir: dataRoot,
    env,
...
/** The store lives at `<dataRoot>/opencode/opencode.db`, so `dataRoot` is the share dir. */
export function sourceFor(dbPath: string, table: string): SourceSpec {
  const path = `${dbPath}#${table}`
  return { id: deriveSourceId(AGENT_ID, path), path: dbPath, kind: 'sqlite', sqliteTable: table, sessionHint: null }
}

/**
 * Snapshots pin event ids, and ids are derived from the source id — so the fixture's
 * random `mkdtemp` path must not leak into them. `SNAPSHOT_DB_PATH` stands in for the
 * path while the rows are still read from the throwaway file.
 */
export const SNAPSHOT_DB_PATH = '/agentlens-opencode-fixture/opencode.db'
...
export function ctxFor(source: SourceSpec, sessionHint: string | null = null): NormalizeCtx {
  return {
    source,
    agentId: AGENT_ID,
    hostId: AGENT_ID,
...
export async function scanSource(
  ...
  const stream = openCodeAdapter.parse(source, { offset: fromRowid }, ctx)
  while (true) {
    ...
    if (next.done) {
      ...
      for (const record of records) {
        const result = await openCodeAdapter.normalize(record, ctx)
