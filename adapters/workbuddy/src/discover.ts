 ...
 *
 * Deliberately absent: a `kind: 'sqlite'` source for `workbuddy.db`. The collector's
 * SQLite path calls `readSqliteIncremental`, which opens the file with `node:sqlite` —
 * and a read-only open of this WAL store is what created `-wal`/`-shm` in the user's data
 * directory (§二, §18 row 7). Advertising a source the collector would open is the
 * side effect, so the database is reported as a refusal by `sqlite.ts` instead, and
 * `detect()` carries that into doctor. See the note in index.ts on where the guard
 * properly belongs.
 */
