<script lang="ts">
  ...
  let {
    ...
    kind = null,
    since = Date.now(),
    loadingText = msg('states.loading'),
...
{:else if status === 'error'}
  <div class="my-4 flex gap-3 rounded-card bg-surface p-4 text-sm shadow-card" role="alert">
    ...
    <div class="min-w-0">
      <div class="font-semibold text-ink">{$t('states.requestFailed')}{#if kind}<span class="ml-1.5 font-normal text-ink-3">· {kind}</span>{/if}</div>
