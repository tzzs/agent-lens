...
5. 🟡 **`parent_event_id` 全 NULL（3,433/3,433 子代理事件）——机制已出现，但不能压在流沙上**。仓库本日新增 `packages/storage/src/subagent-parent-links.ts`：摄入后从**已持久化的行**反推父链，适配器只需声明词表（`SubagentLinkVocabulary`），且它明确拒绝在适配器里做（那会让答案取决于扫描顺序，违反 §4.2）。这正是 zcode 该走的路，但：(a) 该文件此刻仍是**另一会话未提交的在改文件**；(b) zcode 要说这门语言得先把 `Agent` 部件映射成 `tool.start`+capability `subagent`（候选池的查询条件是 `type='tool.start' AND capability_type='subagent' AND metadata[spawnToolUseKey]`，而 §七 现在产出的是 `subagent.start`）；(c) 证明源在盘上现成——`cli/agents/<sess>/agent_*/metadata.json` 同带 `childSessionId` 与 `parentToolUseId`，是 foreign-key 级证据而非启发式。等 (a) 落地后再定 (b)(c)，届时这条才谈得上"修"。
