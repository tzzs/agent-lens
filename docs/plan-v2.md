...
- **本轮审计还欠着的四处（已排期，不是结论）**：(1) §14 两处数值分歧 —— served doctor 的 `priceTableSize` 不叠加 `pricing-overrides.jsonl`（CLI 报 433 有价/6 缺，`/api/doctor` 报 432/11），以及非 CLI 入口起服务时 `billingModeFor` 只认注入、落回 `'api'`，于是 POST 计费声明成功但 `/api/overview` 数字不动；(2) `--explain` 缺 §19 承诺的"无日期价格追溯生效"告警（只有 doctor 有）；(3) `agl export --limit N` 被接受且被忽略（实测 `--limit 5` 仍推 370,493 个 span）、`agl sessions` 仍印 project digest；(4) §4.1 项目标签对既有库不闭环 —— canonical_root 只在摄入时写、已扫完的源被 `skip`，所以老库里 44/50 行仍是摘要，需要回填路径。**§2 的 Machine 实体仍未落地**（今天没有任何机器概念，`agents` 表只有 id/display_name/detected_version/data_root/last_seen_at），它排在上面几项之后：先修数字一致性，再动 schema。
