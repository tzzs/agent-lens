...
- **立方体在全历史路由上的量级：已落地为持久 stage 1（2026-09-23，d5e8933 读路径 / 38f412e 写路径 / 65166c8 体检）**。原判断是"一次全历史 `/api/projects` 至少 4 遍折叠 ⇒ ~34 s，方向是把 §18 的 stage-1 落成持久表"。现在表落地了：迁移 `008_persisted_request_fold.sql` 建 `requests(agent_key, req_key, rep_id, 折叠值, 跨度证书, 代表行的维度列)` + `requests_state`，`migrate()` 里 backfill（§6：升级不要求任何人重扫），写路径的 §5.3 修复、`prune`、`setAgentAggregations` 三处增量维护，`SNAPSHOT_TABLES` 收进两张表所以既有幂等断言顺带断言了派生表。**先记账一条被推翻的前提**：~34 s 已经不是现状 —— 本机库长到 398,794 事件后，"每 scope 只物化一遍"（`fold-cache.ts`）+ 006 的那条索引 + pragma 已把它压到 ~6 s，这条理由本身不再支撑"必须持久化"，持久化的理由只剩"CLI 与全历史第一遍"。测量（`docs/research/probe-request-fold.mjs`，APFS 克隆 `/tmp`，两臂交错，best-of-5，七条路由逐字节相同）：三条 **declined** 路由（窗口 + 有 fold scope，两臂跑同一份代码）给出 x0.88 / x0.90 / x0.94，即本机噪声带约 ±10%；真正走上表的四条是 `/api/projects` 全历史 6167 → 4717 ms（**x1.31**）、`usage --by model` x1.53、`usage --by day` x1.20、`cube projects` x1.12，全部在噪声带之上。写路径代价：20,000 行重摄入 736/804 ms → 950/1015 ms（**+26~29%**，16,015 个组）。**一次方法论自打脸**：best-of-2 时 `/api/projects (full)` 读作 x0.80"更慢"，我把同一探针跑到 best-of-5 才看清它是 x1.31 —— 没有空跑对照的比值在这台负载 ~20 的机器上不构成结论（与 §19 早先"断言 = 被测常量等于没测"同族：没量噪声带的加速比等于没量）。读的三条判据写在 `packages/query/src/persisted-fold.ts`：表里的分组指纹必须等于调用方的 §18 map、过滤必须限于 `agent`/窗口且窗口必须整组在内（`ts_count = member_count AND min_ts >= ? AND max_ts <= ?`，跨界者一律整条 decline）、`SUM(member_count)` 必须仍等于 `COUNT(*) FROM events`；`project`/`session`/`model`/`status`/`type`/`metadata`（含 §18 的 subagent 开关）任一项出现就现折。**为什么窗口不走上表**：强行走实测更慢（`/api/projects?since=30d` 2.0 → 2.2 s、`/api/overview?since=30d` 2.6 → 3.1 s），因为物化的临时表只装窗口内的组，而表要扫全量 36.7 万行 —— 所以有 scope 可复用时就 decline，CLI（无 scope）与全历史才吃这个赢面。表没有外键兜底（`rep_id` 是一个组的 MAX(id)，不是任一事件的孩子），所以 `prune` 删除后扫掉失去代表行的组，`requestFoldHealth`/`requestFoldSentence` 把"是否仍描述 `events`"印给 CLI、`/api/doctor` 与 Doctor 页同一句；**红的是速度不是数字** —— decline 之后一切照旧现折。仍欠：全历史 `/api/projects` 的大头已经不在 stage 1，而在 stage 2 与路由自身的多次扫描。
- **stage 2 的分账与第一次削减（2026-09-24，`docs/research/probe-stage2-cost.mjs`，APFS 克隆 398,794 事件，best-of-5，每臂一个全新 fold-cache scope）**：一次全历史 `/api/projects` 拆开来是 `top`(project 粒度，10 个 metric) **2.40 s** + `sub`(project×agent) 1.06 s + `modelMix` 0.51 s + `capMix` 0.23 s + `costView`(agent 粒度) 1.17 s + `json_extract($.cwd)` 分组 0.12 s + sessions 读 0 ms，整条路由 **4.43 s**。两件事由此定下来：**(1) `costView` 那一趟是给一个没人渲染的字段付的钱** —— `ProjectsResponse.cost` 在 `api.ts` 里有类型、Projects 页只读 `rows/truncated/noteCode`，删掉之后路由 4.43 → **3.44 s（−22%）**，并留下 `packages/server/test/projects.test.ts` 用 `foldPasses()` 把"这个路由做几次会折叠的读"钉成 3（`capMix` 只要 `events`，根本不进 stage 1，所以是 3 不是 4 —— 按实测写，不按直觉写）。**(2) 我先前那句"四次调用合成一次带多维分组再切片"是不安全的**，被数字逼出来的结论：`sessions` 是 COUNT(DISTINCT)（虽然 `sessions.agent_id` 让"一个会话只属于一个 agent"成立，但这条规则今天谁都没说过），cost 两列又是 §18 row 1 的 NULL 保序融合 —— 在路由里 JS 重切片等于把这两条规则从引擎搬进 surface，正是 §14 记过四次的那类裂。真要做，得做成引擎里的 scope 级复用：一次请求内把最细维度的 bucket 读物化进临时表，粗粒度用 SQL 再聚合（bucket 是纯 SUM，融合发生在计价之后，所以语义可证相同），粗粒度调用复用而不是重扫 —— 按同一批数字估约再省 0.6 s（≈18%），而本机 best-of-5 的噪声带是 ±10%，所以这一步要么连同计数器一起落，要么不值得落。
- **先加计数器，再改，然后逐字节 A/B（2026-09-24，上一条说的顺序真跑了一遍）**。第 0 步是 `FoldPassCounters.costBuckets`：一次 `query()` 到底做几遍"计价用的 bucket 读"从此可见，`packages/server/test/route-read-budget.test.ts` 把六条热路由的 (stage-2 读, 计价读) 钉成表。计数器立刻纠正了我自己的猜测：**真正在重复的是"同一趟调用既要 rows 又要 totals"** —— totals 的 bucket 分组就是 rows 的分组抹掉维度，旧代码却把语句跑两遍。改法落在引擎里而不是路由里（§14）：`readBuckets()` 一次读、`priceBucketRows()` 纯计价、`mergeBucketsOverDims()` 只把维度列抹掉再求和，计价与 §8 的 NULL 支配规则原地不动。许可证书是一条性质测试 `packages/query/test/cost-bucket-linearity.test.ts`：分组调用的 totals 必须等于无维度调用的 totals（后者根本不走 merge），跨 8 种价目形状 × 3 种计费模式 × 截断与否，并要求可计价形状真的是数字而不是"两个 NULL 相等"——**我第一版就是这个坑**：夹具里塞了一个无价模型让 totals 恒为 NULL，把 merge 故意改坏（后写覆盖而不是求和）49 条测试照样全绿，加上 numeric 断言后同样的破坏立刻红 18 条、数字是 171.45 vs 342.90（正好少一个项目）。效果：计价读 `projects` 3→2、`agents` 7→6、`models` 6→5、`sessions` 2→1，`overview` 8→8（它的成本读全是 `totals:false`，本来就没翻倍 —— 我原先以为能减半，是错的）。九条路由 HTTP 逐字节对照（同一克隆、同一 server 接线、best-of-5 跑了两轮并交换顺序）：除 `sinceTs` 这个墙钟字段外**全部 sha 相同**；时间 projects-full 3826→3492、agents-full 5618→5291（−6~9%），其余在 ±10% 噪声带内看不出差别 —— 同一份代码两轮之间自己就能差 9%（overview-30d 2663 vs 2917），所以这里只有"读的次数"是硬事实。
- **计数器再往前一步：让每一遍计价读说出是谁要的（2026-09-24，接 `4775a0b`）**。`costBucketSites` 把每遍计价读按 `维度 | 指标` 归位，一跑就看清 `costView` 一个人做 4 遍：agent 粒度两遍（api-equiv 一遍、未上报半边的 cash 一遍），(agent, provider, model) 粒度再两遍 —— 而那两条语句**只差一个 `WHERE r.rep_cost IS NULL`**。于是把"未上报"从 WHERE 改成 GROUP BY 列（`b_unreported`），一遍读物喂两半：`foldBuckets()` 一次遍历做两个折叠（api 折全部桶、cash 只折未上报的桶），§18 row 1"报过的不再计价"与 §8 的 NULL 支配原样保留，没有价表时仍用"什么都查不到"的解析器把未上报切片判成缺口而不是 $0。计价读 `overview` 8→6、`agents` 6→4、`models` 5→3、`doctor` 4→2，`projects`/`sessions`/`capabilities` 不变。九条路由 HTTP 逐字节 A/B（同一克隆、best-of-5、两臂换序）：**九个 sha 全等**。时间给出剂量关系：掉了读数的三条快 6.6%~15.5%（agents-full 5513→4715、models-30d 1928→1629、overview-30d 2782→2595），没掉读数的三条留在 ±10% 噪声带里 —— 效果跟着机制走，才是它真的证据。守卫同时改写：`route-read-budget.test.ts` 在我改完的那一刻红了 4 条（说明它有用），`cost-bucket-linearity.test.ts` 的"一次调用一遍读"从 2 收到 1。
- **中文首轮像素回归（2026-09-24）：上面那两处欠账修掉了，并且只有中文才暴露出第三处**。手法补记（下次少走弯路）：要看 11–12px 的脚注就别截整页，用 `Page.captureScreenshot{clip}` 按元素包围盒截；**每个脚本 run 都 `/json/new` 一个 target 而不关，攒到 7 个（各挂一条 SSE）之后 headless 渲染就慢到截图脚本五分钟不返回**，先 `/json/close` 清干净再跑；**服务端不给 `index.html` 任何 cache 头，Chrome 会一直用缓存里旧 hash 的 bundle**，所以改完 `dist` 必须带 `?v=N` 破缓存，否则结论会是"我改了但它没生效"。三处修复：(1) `DataTable` 的横滚线索 —— 右缘 40px 的 `--ink-3` 渐隐（不是淡到 `--surface`：浅色主题下那是白叠白，等于没有）、首列 `position: sticky`、外加只在真能滚时才出现的 `role=region` + `aria-label` + `tabindex=0`，滚到底渐隐消失（CDP 实测 `scrollLeft=322` 时 `fades:0`、首列仍钉在 `x=16`）。(2) `Sparkline` 的脚注从"日期·说明·日期一行三段"改成"日期一行 + 说明一行"，因为中文那句 `2 个时间桶为"无价格"` 一定比英文长，一行三段的结果是把词从中间劈开。(3) **只有中文会暴露的两处**：`Segmented` 的按钮缺 `whitespace-nowrap`，于是 `24 小时` 竖着排成三行；header 是固定 `h-14` + 中间 `overflow-x-auto no-scrollbar`，pill 一变宽，两个筛选下拉就被推进一条**看不见的横滚**里（手机上看不到"全部 agent/全部主机"存在）。现在 pill 不换行、header 在窄屏换行（430px 下 56 → 85px 高），pills 与两个下拉全部可见。新增 catalog 键 `comps.scrollTable`（en+zh 各一条，parity 测试覆盖）。验证：`tsc` 干净、138 files / 1178 tests、web build，以及 430/1280 两档真像素逐张看过。
...
- **§8 的计费模式改成按 (agent, model) 声明，并给套餐一个价格（2026-09-24）**：原表把 `subscription` 写成"显示 $0"，而 $0 在 §8 里同时是"本地免费模型"的意思 —— 一个用月费买了套餐的人，在仪表盘上看到的现金支出和一个没花钱的人完全一样。用户据此指出"这不符合预期"，并要求把选择放开。落地方式：
  1. **粒度**：`config.json` 的 `billing.<agent>` 现在既能是原来的裸模式字符串（历史声明零改动继续生效），也能是 `{mode, planUsdPerMonth, models:{"<provider>/<name>": mode}}`；解析顺序 **model 覆盖 > agent 默认 > api**，收在 `@agentlens/pricing → billingModeFor/planFeeFor` 一处，立方体的 `priceBuckets` 与 Settings 路由都调它，没有任何面自己去索引那张表（`rule-homes.test.ts` 新增该规则并禁止 surface 解包 `modes[x].mode`）。
  2. **现金**：`actualUsdFor` 从 §8 的第二副本（server 里手写的 `mode === 'api' ? api : 0` + 一个比对测试）合并回 pricing 的唯一实现。逐请求的边际现金仍是 0（这是真的，且 `cost_total` 的未上报半边就按它计价 —— 把它改成 NULL 会把所有订阅 agent 的 §18 融合值打成 NULL，这条是写第一版时读 cube 才发见的）；套餐的钱走 `@agentlens/pricing → planCostFor`（由 `packages/server/src/cost.ts` 的 `planCostForAgent` 按 agent 调用），**按窗口天数摊销、每个 agent 只计一次**（费用买的是套餐本身，两个模型同套餐不能收两遍），未填费用或窗口无界时返回 null 而不是 $0。
  3. **测试抓到的一个真实缺陷**：`createContext` 过去在调用方注入 `billingModeFor` 时就不再读 `config.json`，而 CLI 的 `--serve` 正是注入方 —— 于是页面写进去的套餐费在服务里永远不生效，`planCostUsd` 恒为 null。现在"有没有文件可读"与"有没有人注入视图"解耦。
  4. 展示端：Agents 卡片的模式标签在模型不一致时改为"（混合）"，因为只报默认值等于宣称一个含按量模型的 agent 现金为 0；`agl pricing billing set` 补 `--model` 与 `--fee`，让终端能写页面写得下的同一批事实（§14）。
  5. **顺带挖出一个既有 §14 缺陷（`e1fb68f`）**：`createContext` 为了 `/api/health` 好看，把 `redactHome(dbPath)` 存进 ctx，而 `serve.ts` 又是把这个 ctx 二次交给 `createContext`（经由 `createApp`）——于是所有从 dbPath 派生的路径变成字面量 `~/...`：设置路由读写 `~/config.json`（不存在），而立方体读的是真的那份。**后果是页面里声明的计费模式在 served dashboard 上静默不生效**。测试看不见它，因为 `harness()` 给 `createApp` 的是新鲜 deps、从不二次进入。修法：ctx 上保留真实路径，只在渲染响应时脱敏（`configFile` 对客户端仍是 `~` 形式）。守护测试按默认 `~/.agentlens/agentlens.db` 的形状把 fixture 放进假 home，并做了双向变异检验。
...
- **`agl pricing update` 会把全部历史成本擦成 `n/a`（2026-09-23 修）**：抓下来的 litellm 条目不带 `effective_from`，`normalizeLitellmEntries` 于是让它们继承 `fetchedAt`；而 `pickEffective` 只认 `effectiveFrom <= occurredAt`，所以任何早于这一次抓取的事件都查不到价 —— 用户跑一次"刷新价格"，历史反而全成了未知。真实上游实测：修复前 `lookup('anthropic','claude-sonnet-5', 30 天前)` 返回 `null`、`lookup(..., now)` 返回 `2`；修复后两个都是 `2`，抓下来的 4,476 条全部 `effective_from: 0`。**为什么 1096 个用例全绿还带着它**：唯一的 e2e 把抓取时钟写成 `now: () => T0 - 86_400_000`（事件之前一天），恰好绕开这一步 —— 夹具里的时钟不是随手写的，但它遮住了什么没人问过，与 §19 那条"断言 = 被测常量等于没测"同族。修法不是再定一套语义，而是把构建期生成器早就在用的"无日期"约定搬到抓取路径（两家上游都不发布价格历史）；OpenRouter 兜底源落地时用的正是同一条约定，这个不对称才显得刺眼，新加的 e2e 因此**故意不传 `now`**，让回归必须响。顺带改掉 `explain.ts` 与 `doctor.ts` 里那句"`agl pricing update` fetches dated entries"：它在修复前是假的，不改在修复后仍然是假的 —— 更新只换率，日期只能由 `pricing override` 给。既然"无日期"意味着每个 $ 数字都是"今天的价"，面板现在也说得出这张表多久没动了（`snapshotAgePhrase` 归 pricing，两个 surface 不许各写一份）；上游那份模型列表是会变的（一小时内 444 → 452 条），这也是兜底源只在线取、不打包进构建产物的理由。
- **成本头条丢掉了"能算的那一半"；以及端口没绑上却照样印 URL（2026-09-24 修，浏览器实测撞出来的两件事）**：
  (1) 给 OpenRouter 兜底源做渲染验证时，种子库里三个模型（litellm 有价 / 只有兜底有价 / 两边都没有），Overview 印 `≥ $0.00` 与 `API equivalent ≥ $0.00`，而**同一个响应的模型表**印 $3.00 + $0.20。根因是一条链而不是一个错：立方体按 agent 分组时 §18 row 1 让整组变 NULL（一片没价就作废）→ `costView` 再 `usd([null])` 折成 0 → 头条于是印"已知至少 $0"这种零信息量数字；同时 CLI 的 total 行印 `n/a`，**同一个库两端两个答案**。修法给下限加了中间一档：`costPortionsByAgent`（住在 `@agentlens/query`，和 `costFloor` 并排）按 (agent, model) 重折一次、只加算得出价的部分。边界要说清楚：**per-agent 的 `cost_total` 仍然是 NULL**（§18 row 1 没有被放宽，它回答的是"这个 agent 花了多少"），下限只服务于头条；`*Partial` 与 `unpricedAgents` 语义不变；Overview 那句 tooltip 改了措辞，因为它原来断言"这些 agent 的 token 被排除在总数之外"，而现在被排除的只是没价的那几片。两端都改成走同一个 owner，`apps/cli/test/cost-agreement.test.ts` 比对的是两边**打印出来的字符串**而不是各自调用的函数（种一处违规：把 server 的 api 回退摘掉 → 三个用例立刻红）。实测：半计价窗口 `≥ $1.00 cost (partly unpriced)` 两端一致；彻底没价的窗口仍然 `n/a`，没有退化成 $0。
  (2) `packages/server/src/serve.ts` 里 `server.once('error', () => resolve())` 把 EADDRINUSE **吞掉**：进程没拿到端口，CLI 却照旧打印 URL 并开浏览器，用户看到的是**别人那个实例**的数据。这次是我自己撞上的 —— 第一次渲染验证读到四个库里根本不存在的模型名，看起来完全正常。修法：`ready()` 在绑定失败时 reject（`close()` 顺带不再对从未监听的 socket 调 `close`，那会抛 `ERR_SERVER_NOT_RUNNING`），`serveDashboard` 先 `await ready()` 再报 URL。`packages/server/test/serve-bind.test.ts` 是本仓唯一真占端口的用例（把吞错误的版本种回去，它会红）。**为什么一直没看见**：`helpers.ts` 刻意"永不绑 socket，测试就不会撞端口"——这条纪律挡住了端口竞态，同时也挡住了这类 bug 的可见性；少一个测试面就是一种盲区。遗留：`--port` 不是 CLI 开关（`DEFAULT_PORT 7317` 固定），冲突时只能叫用户先停掉占用者，要不要开 `--port` 待定。
