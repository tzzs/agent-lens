...
- **M4 浏览器验证缺位**：`apps/web` + `packages/server` 的测试全绿，但那些测试是**进程内**的 —— `packages/server/test/query.test.ts` 与 `apps/web/test/capability-dims.test.ts` 用 `createApp` 直接打路由，覆盖的是 API 形状与 `withCapabilityType` 那类口径，页面组件从未被渲染过（无 DOM/浏览器测试、无截图、无点击回归）→ 属**待验证**，不算"完成"。此前"沙箱会挂起已绑定 socket"的说法**已被实测推翻**（loopback `listen` + 自请求正常返回响应），因此缺的只是执行，不是环境能力。
